...
if noise_type == 'PGDA':
    ...
    base_folder = f"Report/MNIST/PGDA/{epsi}/{epsi}/1"
    os.makedirs(base_folder, exist_ok=True)
    count = 0
    for cln_data, true_label in loader:
...
# Generate Gaussian/Speckle noise reports to folder
    ...
    if noise_type == 'Gaussian':
        base_folder = f"Report/MNIST/Gaussian/{epsi}/{epsi}/1"
        os.makedirs(base_folder, exist_ok=True)
        count = 0
        ...
        base_folder = f"Report/MNIST/Speckle/{epsi}/{epsi}/1"
        os.makedirs(base_folder, exist_ok=True)
        count = 0
        for cln_data, true_label in trainloader_noise:
            image_path = os.path.join(base_folder, f"{count}.png")
...
class Cal_f_score:
    ...
    def eval(self, div_name):
        ...
        # a=0, b=1 is chosen in Algorithm 1
        df = pd.DataFrame(D_losses, columns = ['Payment'])
        df.to_csv(f"Results/{noise_type}_{d_ratio}_{epsi}_{div_name}.csv", index = False)
